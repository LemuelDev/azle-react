...
const AdminSignup: React.FC = () => {
    ...
    const checkAdminExists = async (username: string): Promise<boolean> => { // Explicit type
        try {
            ...
            const adminExists = admins.some((admin: { username: string }) => admin.username === username);
    ...
    const handleSignup = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => { // Explicit type
        ...
        // Proceed with signup
        try {
            const response = await axios.post(`${import.meta.env.VITE_CANISTER_URL}/create-admin`, {
                username: username,
                password: password
            });
        ...
        >
            <div className="flex justify-center items-center max-w-[500px] shadow-xl px-10 py-6 backdrop-blur-[6px] backdrop-saturate-[190%] bg-white/40 rounded-[12px] border border-[#d1d5db]/30">
                <div className="grid gap-4 text-center py-6">
                    ...
                    <form onSubmit={handleSignup} className="grid gap-4 items-center text-left px-8">
                        <div className="grid">
